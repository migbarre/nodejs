...
function renderRoomContacts ( contacts ) {
    chatName.html('Room - ' + params.get('room'));

    html = '';
    for ( var i = 0; i < contacts.length; i++) {
        html += '<li data-id="' + contacts[i].id + '">';
        html += '   <div class="d-flex bd-highlight">';
        html += '       <div class="img_cont">';
        html += '           <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img">';
        html += '           <span class="online_icon"></span>';
        html += '       </div>';
        html += '       <div class="user_info">';
        html += '           <span>' + contacts[i].name + '</span>';
        html += '       </div>';
        html += '   </div>';
        html += '</li>';
...
function renderMessage(message, me) {
    ...
    if (!me) {
        ...
        html += '       <span class="msg_time">' + message.name + ' - ' + hour + '</span>';
        html += '   </div>';
        html += '</div>';
    } else {
...
formSend.on('submit', function(e) {
    ...
    socketChat.emit('sendMessage', {
        ...
        message.val('').focus();
        renderMessage(messageSent, true);
    });
});
